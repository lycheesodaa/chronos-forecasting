...
# loop through all datasets in YAML file
for dataset_config in backtest_configs:
    ...
    metrics = {
        "val_loss_per_lr": val_loss_per_lr,
        "mean_val_loss_per_lr": mean_val_loss_per_lr,
        "best_lr": best_lr_str,
        "best_val_loss": best_val_loss,
    }

    # Save to JSON file
    metrics_file = output_metrics_dir / f"{dataset_name}_lr_comparison.json"
    with open(metrics_file, "w") as f:
        json.dump(metrics, f, indent=4)

    print(f"\nLR loss comparison saved to: {metrics_file}")
