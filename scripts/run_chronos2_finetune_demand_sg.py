...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Fine-tune Chronos-2 for demand_sg, then rolling-evaluate.")
	parser.add_argument("--config-file", type=str, default="./configs/demand_datasets.yaml")
	parser.add_argument("--name", type=str, default=None)
	parser.add_argument("--output-dir", type=str, default="./outputs/chronos2/demand_sg_finetune")
...
def select_configs(configs: List[dict], name: Optional[str]) -> List[dict]:
	...
	for cfg in configs:
		...
		if cfg_name.startswith("demand_sg_"):
			selected.append(cfg)
	if not selected:
		raise ValueError("No 'demand_sg_' entry found in the YAML file.")
	return selected
...
def infer_timestamp_column(df: pd.DataFrame, preferred: str) -> str:
	...
	for col in candidates:
		if col in df.columns:
			return col
	raise ValueError("Could not find a timestamp column. Provide --timestamp-column or ensure a standard one exists.")
...
def build_finetune_inputs(context_df: pd.DataFrame, id_column: str, timestamp_column: str, target: str, covariates: List[str]) -> List[Dict[str, Any]]:
	# We treat all covariates as past-known at training time and declare the same names as future covariates with None
	inputs: List[Dict[str, Any]] = []
	for series_id, group in context_df.groupby(id_column):
		group = group.sort_values(timestamp_column)
		tgt = group[target].values
		cov_dict = {col: group[col].values for col in covariates if col in group.columns}
		future_cov_names = {col: None for col in covariates if col in group.columns}
		inputs.append({"target": tgt, "past_covariates": cov_dict, "future_covariates": future_cov_names})
	return inputs


def compute_metrics(pred_df: pd.DataFrame, df_full: pd.DataFrame, id_column: str, timestamp_column: str, target: str) -> pd.DataFrame:
	joined = pred_df.merge(df_full[[id_column, timestamp_column, target]], how="left", on=[id_column, timestamp_column], validate="m:1")
	...
	metrics = joined.groupby(["cutoff"], dropna=False).agg(mae=("abs_err", "mean"), rmse=("squared_err", lambda x: (x.mean()) ** 0.5), mape=("ape", "mean")).reset_index()
	overall = pd.DataFrame({"cutoff": ["OVERALL"], "mae": [joined["abs_err"].mean()], "rmse": [(joined["squared_err"].mean()) ** 0.5], "mape": [joined["ape"].mean()]})
...
def main() -> None:
	...
	for cfg in cfgs:
		base_name = str(cfg.get("name", "demand_sg_run"))
		print(f"\n=== Fine-tune + rolling evaluate: {base_name} ===")
		csv_path = cfg["filename"]
		prediction_length = int(cfg["prediction_length"])  # horizon for training and eval
		...
		# Build fine-tune inputs and fit
		train_inputs = build_finetune_inputs(context_df, args.id_column, ts_col, target_col, covariates)
		print(f"Fine-tuning on {len(train_inputs)} series, steps={args.num_steps}, lr={args.learning_rate}")
		finetuned_pipeline = pipeline.fit(inputs=train_inputs, prediction_length=prediction_length, num_steps=int(args.num_steps), learning_rate=float(args.learning_rate), batch_size=int(args.train_batch_size), logging_steps=int(args.logging_steps))

		# Rolling evaluation, identical to zeroshot scripts
		...
		for series_id, group in df_full.groupby(args.id_column):
			...
			for cutoff in tqdm(cutoffs, desc=f"{base_name} id={series_id}"):
				...
				pred = finetuned_pipeline.predict_df(ctx_df, future_df=None, prediction_length=prediction_length, quantile_levels=quantiles, id_column=args.id_column, timestamp_column=ts_col, target=target_col, predict_batches_jointly=False, batch_size=100)
				pred["cutoff"] = cutoff
				all_preds.append(pred)

		combined = pd.concat(all_preds, ignore_index=True) if all_preds else pd.DataFrame()
		out_csv = os.path.join(args.output_dir, f"{base_name}_ft_rolling_predictions.csv")
		combined.to_csv(out_csv, index=False)
		try:
			out_parquet = os.path.join(args.output_dir, f"{base_name}_ft_rolling_predictions.parquet")
			combined.to_parquet(out_parquet, index=False)
		except Exception:
			out_parquet = None
		print(f"Saved FT rolling predictions to: {out_csv}")
		if out_parquet and os.path.exists(out_parquet):
			print(f"Saved FT rolling predictions to: {out_parquet}")

		if not combined.empty:
			metrics_df = compute_metrics(combined, df_full[[args.id_column, ts_col, target_col]].rename(columns={ts_col: ts_col}), args.id_column, ts_col, target_col)
			metrics_csv = os.path.join(args.output_dir, f"{base_name}_ft_rolling_metrics.csv")
			metrics_df.to_csv(metrics_csv, index=False)
			print(f"Saved FT rolling metrics to: {metrics_csv}")
