...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Run Chronos-2 zero-shot forecasting for demand_sg using configs.", add_help=False)
	parser.add_argument("--config-file", type=str, default="./configs/demand_datasets.yaml")
	parser.add_argument("--name", type=str, default=None)
	parser.add_argument("--output-dir", type=str, default="./outputs/chronos2/demand_sg")
...
def get_device(device_arg: str) -> str:
    if device_arg == "auto":
        try:
            ...
            return "cpu"
    # Pass through explicit device strings like "cuda", "cpu", or "cuda:0"
    return device_arg
...
def main() -> None:
	...
	for cfg in cfgs:
		...
		for series_id, group in df_full.groupby(args.id_column):
			...
			# For each cutoff, run predict_df on all series up to cutoff
